...
const ZONES = [
  { id:1, name:{ar:"خليج الغزوات",fr:"Baie de Ghazaouet",en:"Ghazaouet Bay"},
    lat:35.09, lng:-1.86, base:0.85, dist:{ar:"2 كلم",fr:"2 km",en:"2 km"}, type:"coastal" },
  { id:2, name:{ar:"غرب الميناء",fr:"Ouest du port",en:"West Harbor"},
    lat:35.10, lng:-1.82, base:0.75, dist:{ar:"3 كلم",fr:"3 km",en:"3 km"}, type:"coastal" },
  { id:3, name:{ar:"رأس الحديد",fr:"Cap El Hadid",en:"Iron Cape"},
    lat:35.12, lng:-1.92, base:0.60, dist:{ar:"4 كلم",fr:"4 km",en:"4 km"}, type:"coastal" },
  { id:4, name:{ar:"مرسى بن مهيدي",fr:"Marsa Ben M'hidi",en:"Marsa Ben Mhidi"},
    lat:35.13, lng:-1.78, base:0.65, dist:{ar:"5 كلم",fr:"5 km",en:"5 km"}, type:"coastal" },
  { id:5, name:{ar:"رأس العقبة",fr:"Ras El Akba",en:"Ras El Akba"},
    lat:35.08, lng:-1.97, base:0.70, dist:{ar:"6 كلم",fr:"6 km",en:"6 km"}, type:"coastal" },
  { id:6, name:{ar:"بني صاف",fr:"Beni Saf",en:"Beni Saf"},
    lat:35.30, lng:-1.38, base:0.55, dist:{ar:"5 كلم",fr:"5 km",en:"5 km"}, type:"coastal" },
  { id:7, name:{ar:"وسط الغزوات",fr:"Centre Ghazaouet",en:"Ghazaouet Center"},
    lat:34.82, lng:-1.86, base:0.78, dist:{ar:"30 كلم",fr:"30 km",en:"30 km"}, type:"mid" },
  { id:8, name:{ar:"شمال رأس الحديد",fr:"Nord Cap Hadid",en:"North Cap Hadid"},
    lat:34.75, lng:-1.93, base:0.68, dist:{ar:"40 كلم",fr:"40 km",en:"40 km"}, type:"mid" },
  { id:9, name:{ar:"الخليج الغربي",fr:"Golfe Ouest",en:"West Gulf"},
    lat:34.87, lng:-1.70, base:0.72, dist:{ar:"25 كلم",fr:"25 km",en:"25 km"}, type:"mid" },
  { id:10, name:{ar:"شمال بني صاف",fr:"Nord Beni Saf",en:"North Beni Saf"},
    lat:34.96, lng:-1.38, base:0.58, dist:{ar:"37 كلم",fr:"37 km",en:"37 km"}, type:"mid" },
  { id:11, name:{ar:"المنطقة العميقة",fr:"Zone profonde",en:"Deep Zone"},
    lat:34.52, lng:-1.86, base:0.50, dist:{ar:"63 كلم",fr:"63 km",en:"63 km"}, type:"deep" },
  { id:12, name:{ar:"شمال غرب عميق",fr:"Nord-Ouest profond",en:"Deep NW"},
    lat:34.55, lng:-2.05, base:0.45, dist:{ar:"67 كلم",fr:"67 km",en:"67 km"}, type:"deep" },
  { id:13, name:{ar:"الحافة الشمالية",fr:"Limite Nord",en:"North Edge"},
    lat:34.48, lng:-1.65, base:0.42, dist:{ar:"70 كلم",fr:"70 km",en:"70 km"}, type:"deep" },
];
...
function OceanMap({ zones, selected, onSelect, isPro, lang, fishColor }) {
  ...
  useEffect(() => {
    ...
    bg.addColorStop(0,"#04111e"); bg.addColorStop(1,"#071e36");
    ctx.fillStyle = bg; ctx.fillRect(0,0,w,h);

    // Grid
    ctx.strokeStyle="rgba(0,229,255,0.04)"; ctx.lineWidth=1;
    for(let la=34.5;la<=35.5;la+=0.25){const[,y]=geo2canvas(la,-2,w,h);ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(w,y);ctx.stroke();}
    for(let lo=-2.2;lo<=-1.1;lo+=0.25){const[x]=geo2canvas(35,lo,w,h);ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,h);ctx.stroke();}

    // 70km line
    const off70 = COAST_LINE.map(([la,lo])=>geo2canvas(la-0.63,lo,w,h));
    ctx.beginPath(); ctx.setLineDash([7,5]);
    ctx.strokeStyle="rgba(240,192,64,0.30)"; ctx.lineWidth=1.5;
    off70.forEach(([x,y],i)=>i===0?ctx.moveTo(x,y):ctx.lineTo(x,y));
    ctx.stroke(); ctx.setLineDash([]);

    // Land fill
    ctx.beginPath();
    ctx.moveTo(0,0); ctx.lineTo(w,0);
    const lc=geo2canvas(COAST_LINE[COAST_LINE.length-1][0],COAST_LINE[COAST_LINE.length-1][1],w,h);
    ctx.lineTo(lc[0],lc[1]);
    [...COAST_LINE].reverse().forEach(([la,lo])=>{const[x,y]=geo2canvas(la,lo,w,h);ctx.lineTo(x,y);});
    const fc=geo2canvas(COAST_LINE[0][0],COAST_LINE[0][1],w,h);
    ctx.lineTo(0,fc[1]); ctx.closePath();
    ctx.fillStyle="rgba(10,30,15,0.7)"; ctx.fill();

    // Coast line
    ctx.beginPath(); ctx.strokeStyle="#00e5ff"; ctx.lineWidth=2.5;
    ctx.shadowColor="#00e5ff"; ctx.shadowBlur=5;
    COAST_LINE.forEach(([la,lo],i)=>{const[x,y]=geo2canvas(la,lo,w,h);i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);});
    ctx.stroke(); ctx.shadowBlur=0;

    // Labels
    ctx.textAlign="center";
    ctx.fillStyle="rgba(0,229,255,0.5)"; ctx.font=`bold ${Math.max(9,w*0.013)}px Cairo,sans-serif`;
    const[clx,cly]=geo2canvas(35.22,-1.70,w,h); ctx.fillText("← ساحل تلمسان →",clx,cly);
    ctx.fillStyle="rgba(100,140,180,0.4)"; ctx.font=`${Math.max(8,w*0.011)}px Cairo,sans-serif`;
    const[slx,sly]=geo2canvas(34.38,-1.70,w,h); ctx.fillText("البحر الأبيض المتوسط",slx,sly);
    ctx.fillStyle="rgba(240,192,64,0.35)"; ctx.font=`${Math.max(7,w*0.010)}px Cairo,sans-serif`;
    const[llx,lly]=geo2canvas(34.45,-1.60,w,h); ctx.fillText("◄ حد 70 كلم",llx,lly);

    // Zones
    zones.forEach((z,i)=>{
      const[cx,cy]=geo2canvas(z.lat,z.lng,w,h);
      const isSel=selected===i;

      if(!isPro && z.type!=="coastal") {
        // Blurred/hidden for free users
        ctx.beginPath(); ctx.arc(cx,cy,12,0,Math.PI*2);
        ctx.fillStyle="rgba(30,50,70,0.7)"; ctx.fill();
        ctx.strokeStyle="rgba(100,130,160,0.3)"; ctx.lineWidth=1.5; ctx.stroke();
        ctx.fillStyle="rgba(100,130,160,0.5)";
        ctx.font=`bold ${Math.max(9,w*0.012)}px Cairo,sans-serif`;
        ctx.fillText("🔒",cx,cy+4);
        return;
      }

      const col = probColor(z.prob);
      const fc = fishColor || col.s;
      const r = Math.max(12,w*(z.type==="deep"?0.020:z.type==="mid"?0.025:0.030));

      if(isSel){
        ctx.beginPath(); ctx.arc(cx,cy,r+9,0,Math.PI*2);
        ctx.strokeStyle=col.s; ctx.lineWidth=2;
        ctx.shadowColor=col.s; ctx.shadowBlur=18; ctx.stroke(); ctx.shadowBlur=0;
      }

      ctx.beginPath(); ctx.arc(cx,cy,r,0,Math.PI*2);
      ctx.fillStyle=col.f; ctx.fill();
      ctx.strokeStyle=col.s; ctx.lineWidth=isSel?2.5:1.5; ctx.stroke();

      const pct=Math.round(z.prob*100);
      ctx.fillStyle=col.s;
      ctx.font=`bold ${Math.max(9,w*0.013)}px Cairo,sans-serif`;
      ctx.fillText(`${pct}%`,cx,cy+4);
    });

  }, [zones, selected, size, isPro, fishColor]);
  ...
  const handleClick = useCallback((e)=>{
    ...
    zones.forEach((z,i)=>{
      if(!isPro && z.type!=="coastal") return;
      const[cx,cy]=geo2canvas(z.lat,z.lng,w,h);
      const d=Math.hypot(mx-cx,my-cy);
      const r=Math.max(12,w*0.030);
      if(d<r&&d<minD){minD=d;best=i;}
    });
    onSelect(best===selected?-1:best);
  },[zones,selected,size,isPro,onSelect]);

  return (
    <div style={{position:"relative",width:"100%",borderRadius:10,overflow:"hidden",background:"#04111e"}}>
...
export default function App() {
  ...
  const scoreLabel = s => s>=75?(lang==="ar"?"احتمالية عالية 🎯":lang==="fr"?"Très probable 🎯":"Very likely 🎯")
    :s>=55?(lang==="ar"?"احتمالية جيدة":"Probable":"Likely")
    :s>=35?(lang==="ar"?"متوسطة":"Moyen":"Moderate")
    :(lang==="ar"?"منخفضة":"Faible":"Low");
              ...
              <>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
                  {[
                    ...
                    {icon:"🌿",val:`${envData.chloro.toFixed(2)} mg/m³`,
                      ok:envData.chloro>=(fishKey!=="all"?fish?.chloroMin:0.35)},
                    {icon:"💨",val:`${envData.wind.toFixed(1)} knots`,
